...
* @returns {any}
*/
export function prove(params_ser) {
    const ret = wasm.prove(addHeapObject(params_ser));
...
* @returns {any}
*/
export function prove_vk(params_ser, vk_ser) {
    const ret = wasm.prove_vk(addHeapObject(params_ser), addHeapObject(vk_ser));
    return takeObject(ret);
}
...
async function init(input, maybe_memory) {
    if (typeof input === 'undefined') {
        input = new URL('halo2_ecc_bg.wasm', import.meta.url);
    }
    const imports = getImports();
